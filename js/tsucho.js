...
export function initTsucho(root, sidebarRoot) {
  ...
  /** 土地・家屋の地区別内訳(所在地の頭の地名部分でグループ化し、評価額・固定資産税相当額を集計する)。 */
  function realEstateAreaTableHtml(data, category) {
    const groups = {};
  ...
  function realEstateAssetSectionHtml() {
    ...
    return `<div class="tsucho-section-card category-asset">
      ...
      ${realEstateAreaTableHtml(data, '土地')}
      <p class="empty-hint" style="margin:14px 0 0">🏚 家屋の地区別内訳(評価額・固定資産税相当額)</p>
      ${realEstateAreaTableHtml(data, '家屋')}
    </div>`;
  }
